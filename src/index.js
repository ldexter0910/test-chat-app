...
io.on('connection', (socket) => {
    ...
    socket.on('sendMessage', (message, callback) => {
        ...
        if (!user) {
            return callback('Not in correct room!');
        }
    ...
    socket.on('sendLocation', (geolocationData, callback) => {
        const user = getUser(socket.id);
        if (!user) {
            return callback('Not in correct room!');
        }
        io.to(user.room).emit('locationMessage', generateLocationMessage(user.username,
            `https://google.com/maps?q=${geolocationData.latitude},${geolocationData.longitude}`));
        callback();
    });
